...
- `/api/pubs`: cacheSeconds=60 + revalidate=60 + Cache-Control header — committed on branch perf-assist/pubs-api-cache; push blocked by missing safeoutputs MCP (4th consecutive run)
...
3. COMMITTED - Add cache to `/api/pubs` route + 300ms debounce (branch perf-assist/pubs-api-cache, push BLOCKED - safeoutputs MCP unavailable 4 consecutive runs)
...
- This has happened in 4 consecutive runs — blocking PR creation
