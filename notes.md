...
- Test suite: 34 files, 284 tests (as of 2026-04-16)

## Performance Notes
- `useCountries` hook: module-level cache (merged to main)
- `useBeerTypes` hook: module-level cache (merged to main)
- `/api/pubs`: cacheSeconds=60 + revalidate=60 — committed on branch perf-assist/pubs-api-cache; push BLOCKED by missing safeoutputs MCP (9th consecutive run)
...
3. COMMITTED - Add cache to `/api/pubs` route + 300ms debounce (branch perf-assist/pubs-api-cache, push BLOCKED - safeoutputs MCP unavailable 9 consecutive runs)
...
- This has happened in 9 consecutive runs — blocking PR creation and monthly summary updates
